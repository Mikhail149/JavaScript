...
var scoreProblem = 0; //Счетчик проблем
var maxProblem = 2; //Максимальное количество проблем
 ...
 */
function startGame() {
    //Убираем событие с копи "Старт" чтобы во время игры она не срабатывала
...
function teleport(coord_y, coord_x){
    if (direction == 'x-') {
       return(document.getElementsByClassName('cell-' + (coord_y) + '-' + (FIELD_SIZE_X - 1))[0]);
    }
    else if (direction == 'x+') {
        return(document.getElementsByClassName('cell-' + (coord_y) + '-0')[0]);
    }
    else if (direction == 'y+') {
        return(document.getElementsByClassName('cell-' + (FIELD_SIZE_Y - 1) + '-' + (coord_x))[0]);
    }
    else if (direction == 'y-') {
        return(document.getElementsByClassName('cell-0-' + (coord_x))[0]);
    }
}
...
function haveFood(unit) {
    ...
    document.querySelector(".scoreGame").innerText = "Текущий счет: " + score;
...
function createProblem() {
    ...
    while (!problemCreated) { //пока еду не создали
        // рандом
        ...
        if (!problem_cell_classes.includes('snake-unit') && !problem_cell_classes.includes('food-unit')) {
            ...
            problem_cell.classList.add("problem_" + scoreProblem);
            scoreProblem++;
            if(scoreProblem > maxProblem){
                if(document.querySelector(".problem_" + (scoreProblem - maxProblem - 1))){
                    document.querySelector(".problem_" + (scoreProblem - maxProblem - 1)).classList.remove("problem-unit"); 
                    document.querySelector(".problem_" + (scoreProblem - maxProblem - 1)).classList.remove("problem_" + (scoreProblem - maxProblem - 1)); 
                }
            }
            if(scoreProblem % 5 == 0){
                maxProblem++;
...
function finishTheGame() {
    //Возвращаем событие на кнопку "Старт"
    document.getElementById('snake-start').addEventListener('click', startGame);
    gameIsRunning = false;
